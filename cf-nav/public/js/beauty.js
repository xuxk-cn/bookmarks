...
(function () {
  ...
  try {
    ...
    if (on('noLinkBorder')) body.classList.add('b-noborder');
    ...
    if (on('welcome')) {
      ...
      if (on('weather')) {
        loadWeather().then((w) => {
          ...
          span.textContent = `${w.city ? w.city + ' ' : ''}${w.icon} ${w.temp}°C ${w.desc}`;
          el.appendChild(span);
        }).catch(() => {});
      }
    }
    ...
    async function loadWeather() {
      ...
      return { temp: Math.round(cw.temperature), icon: weatherIcon(cw.weathercode), desc: weatherText(cw.weathercode), city };
    }

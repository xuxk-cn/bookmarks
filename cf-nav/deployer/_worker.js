...
// ─── 后端 Worker 逻辑 ─────────────────────────────────────────────
// GitHub 仓库 raw 内容基地址（deployer 从这里拉取 cf-nav 的静态文件和 worker bundle）
const GITHUB_RAW = 'https://raw.githubusercontent.com/xuxk-cn/bookmarks/master/cf-nav/release';

// cf-nav/release/public/ 下所有需要部署的静态文件列表
const PUBLIC_FILES = [
  '_headers',
  'admin/index.html',
...
async function handlePost(path, request, env) {
  try {
    ...
    if (path === 'accounts') return handleAccounts(body);
    if (path === 'zones') return handleZones(body);
    if (path === 'deploy') return handleDeploy(body);
...
async function uploadStaticFiles(creds, accountId, projectName, log) {
  ...
  for (let i = 0; i < PUBLIC_FILES.length; i += BATCH) {
    ...
    const results = await Promise.all(batch.map(async (path) => {
      const url = `${GITHUB_RAW}/public/${path}?t=${Date.now()}`;
...
async function buildWorkerBundle(log) {
  ...
  const url = `${GITHUB_RAW}/dist/_worker.js?t=${Date.now()}`;
  log('拉取预打包 Worker 文件...');
  const resp = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } });
